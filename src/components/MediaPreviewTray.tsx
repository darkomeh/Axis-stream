...
export default function MediaPreviewTray() {
  const { previewId, triggerSource, closePreview, openPreview } =
    useMediaPreview();
  const {
    user,
    addToWatchlist,
  ...
  useEffect(() => {
    ...
    const loadPreviewData = async () => {
      ...
      try {
        ...
        const media = await movieService.getPlay(
          previewId,
          itemDetails.type === "Series" ? 1 : 0,
          itemDetails.type === "Series" ? 1 : 0,
  ...
  const handleDownload = (url: string) => {
    ...
    const dlTitle = isSeries ? `${details.title} S1 E1` : details.title;
  ...
  const autoDownload = () => {
    ...
    const bestSource =
      details.sources.find((s) => (s.downloadType || s.type) !== "hls") ||
      details.sources[0];
  ...
  const slideImages = useMemo(() => {
    ...
    if (details.cast) {
      const castAvatars = (Array.isArray(details.cast) ? details.cast : [])
        .map((c) => c.avatarUrl || c.avatar)
          ...
          >
                ...
                <>
                  ...
                  {isTrailerEmbed ? (
                    <iframe
                      src={
                        trailerUrl.includes("?")
                          ? `${trailerUrl}&autoplay=${isTrailerSuppressed ? 0 : 1}&mute=1`
                          : `${trailerUrl}?autoplay=${isTrailerSuppressed ? 0 : 1}&mute=1`
                      }
                  ...
                  {isTrailerSuppressed &&
                    ...
                    user && (
                      <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-black/40 backdrop-blur-3xl pointer-events-none">
                        ...
                        <p className="mt-4 text-fluid-xs font-semibold tracking-wide text-white/80">
                          Play Trailer
                        </p>
                      </div>
                    )}
                ...
                <div className="relative w-full h-full">
                  {!user && (
                    <div className="absolute inset-0 z-30 flex flex-col items-center justify-center bg-black/40 backdrop-blur-3xl text-center p-6">
                      <h3 className="text-fluid-xl font-semibold tracking-tight mb-2">
                        Member Only Content
                      </h3>
                      <p className="text-fluid-sm font-bold text-white/60 tracking-wide max-w-[240px]">
                        This item has restricted access. Sign in to see more or
                        stream.
                      </p>
                      <button
                        onClick={() => {
            ...
            <div className="px-fluid py-fluid-sm space-y-6 md:space-y-10">
              ...
              <div className="flex flex-col gap-1.5 md:gap-2">
                  ...
                  {details?.imdbRatingValue && (
                    <>
                      ...
                      <div className="flex items-center gap-1 md:gap-1.5 bg-[#f5c518] px-1.5 py-0.5 rounded overflow-hidden">
                        <span className="text-black font-semibold tracking-tight text-fluid-sm">
                          IMDb
                        </span>
                        <span className="text-black font-semibold text-fluid-xs leading-none">
                          {details?.imdbRatingValue}
                        </span>
                      </div>
                    </>
                ...
                <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-fluid-sm text-gray-400 font-bold tracking-wider">
                  {(Array.isArray(details?.genres)
                    ? details.genres
                    : (details?.genres as any)?.split(",") || []
                  )
                    .slice(0, 4)
                    .map((g: string, i: number, arr: string[]) => (
              ...
              <div className="space-y-4 md:space-y-6">
                <h3 className="text-white font-semibold text-fluid-xs tracking-[3px] md:tracking-[4px]">
                  Starring
                </h3>
                <div className="flex gap-4 md:gap-6 overflow-x-auto no-scrollbar pb-2">
                  {Array.isArray(uniqueCast) &&
                    uniqueCast.slice(0, 10).map((actor: any, index: number) => (
                      <div
                        key={`${actor.id}-${index}`}
                      ...
                      >
                        ...
                        <div className="text-center w-full px-1 md:px-2">
                          <p className="text-white text-fluid-sm font-semibold leading-tight group-hover:text-brand transition-colors line-clamp-1">
                            {actor.name}
                          </p>
              ...
              <div className="flex items-center justify-around md:justify-start gap-6 md:gap-10 py-4 md:py-6 border-t border-white/5">
                ...
                >
                  <div
                    className={`w-11 h-11 md:w-14 md:h-14 flex items-center justify-center rounded-full border-[1.5px] md:border-2 transition-all ${details && isInWatchlist(details.id) ? "bg-brand border-brand shadow-[0_0_20px_rgba(229,9,20,0.4)]" : "bg-white/5 border-white/10 group-hover:border-white/20"}`}
                  >
                  ...
                  <span className="font-semibold text-gray-400 group-hover:text-white tracking-[1.5px] md:tracking-[2px] transition-colors text-fluid-sm">
                    My List
                  </span>
                </button>
                ...
                <button
                  onClick={() => {
                    ...
                    if (navigator.share) {
                      navigator
                        .share({ title: details?.title, url: url })
                        .catch((err) => {
                          if (
                            err.name !== "AbortError" &&
                ...
                <button
                  onClick={() => {
                    handleClose();
                    navigate(`/details/${slugify(details?.title || "")}`);
                  }}
                  className="flex flex-col items-center gap-2 md:gap-3 group transition-transform active:scale-95"
                >
                  <div className="w-11 h-11 md:w-14 md:h-14 flex items-center justify-center rounded-full bg-white/5 border-[1.5px] md:border-2 border-white/10 group-hover:border-white/20 transition-all">
                      ...
                      onClick={() => {
                        if (trailerUrl && videoRef.current) {
                          setTimeout(() => {
                            ...
                            videoRef.current?.play();
                            document
                              .getElementById("media-preview-modal")
            ...
            <div className="grid grid-cols-1 gap-4">
              {playData?.sources && playData.sources.length > 0 ? (
                playData.sources.map((source, idx) => {
                  ...
                  const dlSize =
                    sourceSizes[downloadTargetUrl] || estimatedSize;
                    ...
                    >
                      <div className="flex items-center gap-5">
                        ...
                        <div className="text-left">
                          <p className="font-bold text-white text-fluid-lg">
                            {source.quality}
                          </p>
