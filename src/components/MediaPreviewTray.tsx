...
export default function MediaPreviewTray() {
 ...
 const media = await movieService.getPlay(previewId, itemDetails.type === 'Series' ? 1 : 0, itemDetails.type === 'Series' ? 1 : 0, itemDetails.detailPath, itemDetails.title, itemDetails.year, itemDetails.type ? String(itemDetails.type) : undefined);
 setPlayData(media);
 } catch (err) {
 ...
 <div className="relative aspect-video w-full bg-transparent overflow-hidden group">
 {loading ? (
 ...
 aria-hidden="true"
 />
 {videoBuffering && !isTrailerEmbed && !isTrailerSuppressed && !trailerEnded && (
 <div className="absolute inset-0 flex items-center justify-center bg-black/75 z-20 pointer-events-none transition-all duration-300">
 <div className="w-12 h-12 border-4 border-brand border-t-transparent rounded-full animate-spin" />
 </div>
 )}
 {isTrailerEmbed ? (
 ...
 className="w-full h-full border-none relative z-10 shadow-[0_0_100px_rgba(0,0,0,0.5)] bg-transparent"
 ...
 <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-black/40 backdrop-blur-3xl pointer-events-none">
 <button 
 onClick={(e) => {
 e.stopPropagation();
 if (videoRef.current) {
 ...
 <p className="mt-4 text-fluid-xs font-semibold tracking-wide text-white/80">Play Trailer</p>
 </div>
 )}
 {!isTrailerEmbed && !trailerEnded && (
 <button
 onClick={() => setIsMuted(prev => !prev)}
 className="absolute bottom-4 sm:bottom-6 right-4 sm:right-6 z-40 w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-black/40 backdrop-blur-3xl border border-white/20 flex items-center justify-center text-white hover:bg-white/20 transition-all hover:scale-110"
 >
 {isMuted ? <VolumeX className="w-5 h-5 sm:w-6 sm:h-6" /> : <Volume2 className="w-5 h-5 sm:w-6 sm:h-6" />}
 </button>
 )}
 {!isTrailerEmbed && trailerEnded && (
 <div className="absolute inset-0 bg-black/40 backdrop-blur-3xl flex flex-col items-center justify-center gap-4 z-30">
 <button 
 onClick={() => { setTrailerEnded(false); videoRef.current?.play(); }} 
 className="flex items-center gap-2 text-white font-semibold tracking-wide text-fluid-lg hover:text-brand transition-colors"
 >
 <Play className="w-5 h-5"/> Replay
 </button>
 <button 
 onClick={handlePlay}
 className="flex items-center gap-2 text-black bg-brand px-6 py-2 rounded-full font-semibold tracking-wide text-fluid-lg"
 >
 Watch Now
 </button>
 </div>
 ...
 {!user && (
 <div className="absolute inset-0 z-30 flex flex-col items-center justify-center bg-black/40 backdrop-blur-3xl text-center p-6">
 <h3 className="text-fluid-xl font-semibold tracking-tight mb-2">Member Only Content</h3>
 <p className="text-fluid-sm font-bold text-white/60 tracking-wide max-w-[240px]">This item has restricted access. Sign in to see more or stream.</p>
 <button 
 onClick={() => { handleClose(); navigate('/profile'); }}
 className="mt-4 px-6 py-2 bg-brand text-white rounded-full font-semibold tracking-wide text-fluid-sm"
 >
 Sign In
 </button>
 </div>
 ...
 <button 
 onClick={handleClose}
 className="w-10 h-10 flex items-center justify-center bg-black/40 backdrop-blur-3xl hover:bg-black/40 backdrop-blur-3xl rounded-full text-white transition-all"
 >
 <X className="w-6 h-6" />
 </button>
 </div>
 ...
 className="w-8 h-8 md:w-10 md:h-10 flex items-center justify-center bg-black/40 backdrop-blur-3xl hover:bg-black/40 backdrop-blur-3xl rounded-full text-white transition-all"
 >
 ...
 >
 <h2 className="text-fluid-lg font-semibold tracking-tight text-white drop-shadow-2xl mb-2 md:mb-4 max-w-[90%] md:max-w-[70%] line-clamp-2 md:line-clamp-none">
 {details.title}
 </h2>
 </motion.div>
 )}
 ...
 <span className="px-1.5 py-0.5 rounded border border-white/20 font-semibold tracking-wide text-gray-300 text-fluid-sm">
 {details.rating}
 </span>
 </>
 )}
 ...
 <span className="text-black font-semibold tracking-tight text-fluid-sm">IMDb</span>
 <span className="text-black font-semibold text-fluid-xs leading-none">{details?.imdbRatingValue}</span>
 </div>
 </>
 )}
 </div>
 ...
 <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-fluid-sm text-gray-400 font-bold tracking-wider">
 ...
 <p className="text-white/80 leading-relaxed font-medium max-w-3xl line-clamp-3 md:line-clamp-none text-fluid-lg">
 ...
 <button 
 onClick={handlePlay}
 className="flex-1 flex items-center justify-center gap-2 md:gap-3 bg-brand text-white py-3.5 md:py-4 rounded-xl font-semibold tracking-wide md:tracking-wide shadow-[0_10px_30px_rgba(229,9,20,0.3)] active:scale-[0.98] transition-all text-fluid-sm"
 >
 ...
 <p className="text-white text-fluid-sm font-semibold leading-tight group-hover:text-brand transition-colors line-clamp-1">{actor.name}</p>
 <p className="text-gray-500 font-bold mt-1 tracking-tight leading-tight whitespace-normal break-words line-clamp-1 text-fluid-sm">
 ...
 >
 <div className={`w-11 h-11 md:w-14 md:h-14 flex items-center justify-center rounded-full border-[1.5px] md:border-2 transition-all ${ details && isInWatchlist(details.id) ? "bg-brand border-brand shadow-[0_0_20px_rgba(229,9,20,0.4)]" : "bg-white/5 border-white/10 group-hover:border-white/20" }`}>
 ...
 <span className="font-semibold text-gray-400 group-hover:text-white tracking-[1.5px] md:tracking-[2px] transition-colors text-fluid-sm">My List</span>
 </button>
 ...
 <span className="font-semibold text-gray-400 group-hover:text-white tracking-[1.5px] md:tracking-[2px] transition-colors text-fluid-sm">Share</span>
 </button>
 ...
 <span className="font-semibold text-gray-400 group-hover:text-white tracking-[1.5px] md:tracking-[2px] transition-colors text-fluid-sm">Details</span>
 </button>
 </div>

 {/* Recommendations Section */}
 <div className="space-y-4 md:space-y-6 pt-6 md:pt-10 border-t border-white/5">
 <h3 className="text-white font-semibold text-fluid-xs tracking-[3px] md:tracking-[4px]">Recommendations</h3>
 ...
 >
 <div className="aspect-[2/3] rounded-lg md:rounded-xl overflow-hidden bg-black/40 backdrop-blur-3xl relative shadow-2xl border border-white/5 group-hover:border-brand/50 transition-colors">
 <MovieImage 
 src={item.poster} 
 alt={item.title} 
 ...
 <div className="space-y-4 md:space-y-6 pt-6 md:pt-10 border-t border-white/5">
 <h3 className="text-white font-semibold text-fluid-xs tracking-[3px] md:tracking-[4px]">Stills & Trailer</h3>
 ...
 <div 
 key={i} 
 className="flex-none w-[220px] md:w-[280px] aspect-video rounded-lg md:rounded-xl overflow-hidden glass-panel cursor-pointer group"
 onClick={handlePlay}
 >
 ...
 <div 
 className="relative flex-none w-[220px] md:w-[280px] aspect-video rounded-lg md:rounded-xl overflow-hidden glass-panel group cursor-pointer"
 onClick={() => {
 ...
 <p className="font-bold text-white text-fluid-lg">{source.quality}</p>
 <p className="text-fluid-xs text-gray-500 tracking-wide mt-1 font-medium">
 ...
 <div className="p-8 text-center text-gray-500 font-bold tracking-wide">
