...
export default function VideoPlayer({
 ...
 // Let progress and playback handlers clear the loader when the content actually plays or buffers
 ...
 <video
 ref={videoRef}
 className={`w-full h-full transition-all duration-300 relative z-10 ${ zoom === "cover" ? "object-cover" : zoom === "fill" ? "object-fill" : "object-contain" }`}
 playsInline
 ...
 <button
 onClick={() => seekTo(120)}
 className="flex items-center gap-2 bg-black/40 backdrop-blur-3xl hover:bg-white text-white hover:text-black border border-white/20 backdrop-blur-md px-5 py-2.5 rounded-full font-bold tracking-wider text-fluid-xs transition-all shadow-[0_0_20px_rgba(0,0,0,0.5)]"
 >
 ...
 className="flex items-center gap-2 bg-brand hover:bg-white text-white hover:text-black hover:border-white border border-brand backdrop-blur-md px-6 py-3 rounded-full font-semibold tracking-wider text-fluid-sm transition-all shadow-[0_0_30px_rgba(229,9,20,0.6)]"
 >
 ...
 className={`absolute top-1/2 -translate-y-1/2 z-30 pointer-events-none ${ gestureFeedback.side === "left" ? "left-1/4" : gestureFeedback.side === "right" ? "right-1/4" : "left-1/2 -translate-x-1/2" }`}
 >
 <div className="flex flex-col items-center gap-2 bg-black/40 backdrop-blur-3xl rounded-full px-6 py-4 border border-white/10">
 ...
 <span className="text-white font-semibold text-fluid-xl">
 {gestureFeedback.type === "forward"
 ...
 animate={{ opacity: 1 }}
 exit={{ opacity: 0 }}
 className={`absolute inset-0 z-40 flex flex-col justify-between pointer-events-none ${useIframeFallback ? "bg-black" : "bg-gradient-to-t from-black/80 via-transparent to-black/60"}`}
 >
 {/* Top Bar */}
 ...
 <h1 className="text-fluid-sm font-semibold text-white truncate drop-shadow-lg tracking-wide opacity-80">
 {title}
 </h1>
 </div>
 </div>
				...
				{!useIframeFallback && (
					<div className="flex items-center gap-2 md:gap-4">
						<button
							onClick={() => {
								if (navigator.share) {
									navigator.share({ title, text: `Check out ${title} on Axis TV!`, url: window.location.href }).catch(() => {});
								} else {
						...
						>
							...
							<span className="font-semibold tracking-tight text-fluid-xs">Shortcuts</span>
						</button>
					</div>
				)}
 </div>
 ...
 className="absolute inset-0 z-50 flex items-center justify-center pointer-events-auto bg-black/40 backdrop-blur-3xl"
 onClick={() => setShowHelp(false)}
 >
 <div
 className="glass-panel border border-white/10 rounded-2xl p-5 sm:p-8 max-w-lg w-[92%] sm:w-full text-white shadow-2xl relative"
 ...
 <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-y-4 sm:gap-x-8 text-fluid-sm">
 <div className="flex justify-between items-center bg-white/5 p-3 rounded-lg">
 <span className="font-bold text-gray-400">
 Play / Pause
 </span>{" "}
 <span className="bg-white/10 px-2 flex items-center h-6 rounded font-mono text-fluid-xs">
 Space
 </span>
 </div>
 <div className="flex justify-between items-center bg-white/5 p-3 rounded-lg">
 ...
 >
 <div className="relative flex items-center justify-center w-10 sm:w-12 md:w-14 h-10 sm:h-12 md:h-14 rounded-full border border-white/20 bg-black/40 backdrop-blur-3xl group-hover:bg-white/10">
 <RotateCcw className="w-4 h-4 md:w-8 md:h-8 text-white stroke-[1.5]" />
 <span className="absolute font-semibold text-white mt-0.5 text-fluid-sm">
 10
 </span>
 </div>
 </button>
 ...
 <button
 onClick={togglePlay}
 className="w-14 sm:w-16 md:w-20 lg:w-24 h-14 sm:h-16 md:h-20 lg:h-24 flex items-center justify-center bg-[#E50914] rounded-full text-white hover:scale-105 active:scale-95 transition-all shadow-[0_0_50px_rgba(229,9,20,0.4)] relative group"
 >
 {isPlaying ? (
 <Pause
 ...
 <div className="text-white font-mono text-fluid-xs tracking-wider min-w-[60px] md:min-w-[70px] text-right shrink-0">
 ...
 <span className="hidden sm:inline text-white/40 text-fluid-xs font-semibold tracking-tight">
 1X
 </span>
 <button
 ...
 className="absolute top-4 right-4 z-50 p-3 bg-black/40 backdrop-blur-3xl rounded-full border border-white/20 text-white"
 >
 ...
 {/* Settings Modals - Minimal Netflix-style panel on the Right */}
 <AnimatePresence>
 {activeMenu && (
 <div
 className="absolute inset-0 z-50 flex items-center justify-end p-2 pointer-events-none"
 onClick={() => setActiveMenu(null)}
 >
 {/* Minimal dark backdrop - only visible on small screens or just subtle */}
 <motion.div 
 initial={{ opacity: 0 }}
 animate={{ opacity: 1 }}
 exit={{ opacity: 0 }}
 className="absolute inset-0 bg-black/40 backdrop-blur-3xl-[2px] sm:bg-black pointer-events-auto" 
 />
 ...
 onClick={(e) => e.stopPropagation()}
 className="relative w-[min(90vw,260px)] sm:w-[260px] h-fit bg-black/40 backdrop-blur-3xl rounded-[8px] overflow-hidden shadow-[0_10px_30px_rgba(0,0,0,0.5)] border border-white/5 pointer-events-auto ring-1 ring-white/5"
 >
 ...
 >
 <span className="text-fluid-xs font-medium text-white group-hover:text-white transition-opacity">{item.label}</span>
 <div className="flex items-center gap-2">
 <span className="text-fluid-sm font-normal text-[#aaa] group-hover:text-white transition-colors">{item.value}</span>
 <ChevronDown className="w-3 h-3 -rotate-90 text-[#aaa] group-hover:text-white transition-colors" />
 </div>
 </button>
 ))}
 ...
 <span className="text-fluid-xs font-medium text-white">Data Saver</span>
 </div>
 ...
 className="w-full flex items-center justify-between px-4 py-2 hover:bg-white/10 active:bg-white/15 transition-all group"
 >
 <span className="text-fluid-xs font-medium text-white">Season & Episode</span>
 <div className="flex items-center gap-2">
 <span className="text-fluid-sm font-normal text-[#aaa]">S{selectedSeason} E{selectedEpisode}</span>
 <ChevronDown className="w-3 h-3 -rotate-90 text-[#aaa]" />
 </div>
 </button>
 )}
 ...
 className="w-full flex items-center justify-between px-4 py-2 hover:bg-white/10 active:bg-white/15 transition-all group"
 >
 <span className="text-fluid-xs font-medium text-white/90">Download</span>
 <div className="text-[#E50914] flex items-center">
 ...
 <span className="text-fluid-xs font-bold tracking-wide text-white/30">Episodes</span>
 </div>

 <button 
 onClick={() => setActiveMenu("seasons")}
 className="w-full flex items-center justify-between px-4 py-2 bg-white/5 group hover:bg-white/10 transition-colors"
 >
 <div className="flex flex-col items-start">
 <span className="text-fluid-xs font-semibold text-white/30 tracking-wide">Season</span>
 <span className="text-fluid-xs font-semibold text-white group-hover:text-brand transition-colors">Season {viewingSeason.toString().padStart(2, '0')}</span>
 </div>
 <ChevronDown className="w-3 h-3 text-white/30 group-hover:text-brand transition-colors" />
 </button>

 <div className="flex flex-col py-1 max-h-[40vh] overflow-y-auto no-scrollbar">
 ...
 className={`w-full flex items-center justify-between px-4 py-2 hover:bg-white/5 transition-colors ${isCurrent ? 'bg-brand/10 text-brand' : 'text-white/70'}`}
 >
 <div className="flex items-center gap-2">
 <span className={`text-fluid-xs font-mono tabular-nums ${isCurrent ? 'text-brand' : 'text-white/20'}`}>
 {epNum.toString().padStart(2, '0')}
 </span>
 <span className="text-fluid-xs font-bold truncate">Episode {epNum}</span>
 </div>
 {isCurrent && <Check className="ml-auto w-3 h-3" />}
 </button>
 );
 })}
 </div>
 ...
 <span className="text-fluid-sm font-semibold tracking-[3px] text-white/40">Select Season</span>
 </div>
 ...
 className={`w-full flex items-center justify-between px-6 py-4 hover:bg-white/5 transition-colors ${isCurrent ? 'bg-brand/10 text-brand' : 'text-white/70'}`}
 >
 <div className="flex flex-col items-start">
 <span className="text-fluid-sm font-semibold ">Season {s.se}</span>
 <span className="text-fluid-xs font-bold text-white/30 ">{s.maxEp} Episodes</span>
 </div>
 ...
 <span className="text-fluid-xs font-bold tracking-wide text-white/30">Caption Style</span>
 </div>
 <div className="p-4 space-y-6">
 <section className="space-y-3">
 <label className="text-fluid-sm font-semibold text-white/30 tracking-wide">Size</label>
 <div className="grid grid-cols-4 gap-2">
 {[14, 18, 22, 28].map((size) => (
 <button
 key={size}
 onClick={() => updateSubtitleSetting({ fontSize: size })}
 className={`py-2 rounded-xl transition-all border font-bold text-fluid-xs ${subtitleSettings.fontSize === size ? "bg-brand border-brand text-white" : "bg-white/5 border-white/5 text-white/60 hover:bg-white/10"}`}
 >
 ...
 <section className="space-y-3">
 <label className="text-fluid-sm font-semibold text-white/30 tracking-wide">Color</label>
 ...
 <section className="space-y-3">
 <label className="text-fluid-sm font-semibold text-white/30 tracking-wide">Background Opacity</label>
 <div className="grid grid-cols-4 gap-2">
 ...
 className={`py-2 rounded-xl transition-all border font-bold text-fluid-sm ${isSelected ? "bg-brand border-brand text-white" : "bg-white/5 border-white/5 text-white/60 hover:bg-white/10"}`}
 >
 ...
 <span className="text-fluid-xs font-bold tracking-wide text-white/30">Audio Track</span>
 </div>
 ...
 className={`w-full flex items-center justify-between px-4 py-2 hover:bg-white/5 transition-colors ${isSelected ? "text-brand bg-brand/5" : "text-white/70"}`}
 >
 <span className={`text-fluid-xs ${isSelected ? 'font-semibold ' : 'font-medium'}`}>{track.displayName || track.language || track.name || `Track ${idx + 1}`}</span>
 ...
 <span className="text-fluid-xs font-bold tracking-wide text-white/30">Captions</span>
 </div>
 <div className="flex flex-col py-1">
 <button
 onClick={() => {
 ...
 className={`w-full flex items-center justify-between px-4 py-2 hover:bg-white/5 transition-colors ${currentSubtitleTrack === -1 ? "text-brand bg-brand/5" : "text-white/70"}`}
 >
 <span className={`text-fluid-xs ${currentSubtitleTrack === -1 ? 'font-semibold ' : 'font-medium'}`}>Off</span>
 {currentSubtitleTrack === -1 && <Check className="w-3 h-3" />}
 </button>
 ...
 className={`w-full flex items-center justify-between px-4 py-2 hover:bg-white/5 transition-colors ${currentSubtitleTrack === idx ? "text-brand bg-brand/5" : "text-white/70"}`}
 >
 <span className={`text-fluid-xs ${currentSubtitleTrack === idx ? 'font-semibold ' : 'font-medium'}`}>{track.displayName || track.language || track.name || `Track ${idx + 1}`}</span>
 {currentSubtitleTrack === idx && <Check className="w-3 h-3" />}
 </button>
 ))}
 </div>
 </div>
 ...
 <span className="text-fluid-xs font-bold tracking-wide text-white/30">Playback Speed</span>
 </div>
 <div className="flex flex-col py-1">
 ...
 className={`w-full flex items-center justify-between px-4 py-2 hover:bg-white/5 transition-colors ${playbackSpeed === speed ? "text-brand bg-brand/5" : "text-white/70"}`}
 >
 <span className={`text-fluid-xs ${playbackSpeed === speed ? 'font-semibold ' : 'font-medium'}`}>{speed === 1 ? "Normal" : `${speed}x`}</span>
 ...
 <span className="text-fluid-xs font-bold tracking-wide text-white/30">Quality</span>
 </div>
 <div className="flex flex-col py-1">
 {hlsLevels.length > 0 ? (
 <>
 ...
 className={`w-full flex items-center justify-between px-4 py-2 hover:bg-white/5 transition-colors ${hlsCurrentLevel === -1 ? "text-brand bg-brand/5" : "text-white/70"}`}
 >
 <div className="flex flex-col items-start">
 <span className={`text-fluid-xs ${hlsCurrentLevel === -1 ? 'font-semibold ' : 'font-medium'}`}>Auto</span>
 <span className="text-fluid-xs text-white/30 tracking-wide font-semibold">Rec</span>
 </div>
 {hlsCurrentLevel === -1 && <Check className="w-3 h-3" />}
 </button>
 ...
 className={`w-full flex items-center justify-between px-4 py-2 hover:bg-white/5 transition-colors ${hlsCurrentLevel === idx ? "text-brand bg-brand/5" : "text-white/70"}`}
 >
 <span className={`text-fluid-xs ${hlsCurrentLevel === idx ? 'font-semibold ' : 'font-medium'}`}>{level.height}p</span>
 {hlsCurrentLevel === idx && <Check className="w-3 h-3" />}
 </button>
 ))}
 </>
 ) : (
 ...
 <span className="text-fluid-base font-semibold tracking-wide text-[#333] dark:text-white/40">
 Report Issue
 </span>
 </div>
 <div className="py-2">
 ...
 className="w-full flex items-center gap-3 px-5 py-3 hover:bg-black/40 backdrop-blur-3xl dark:hover:bg-white/5 transition-colors text-left"
 >
