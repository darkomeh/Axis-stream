    ...
    <div className="bg-white/5 border border-white/10 rounded-[2.5rem] p-8 space-y-8">
      <div className="flex items-center justify-between border-b border-white/10 pb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-black italic uppercase tracking-tighter">API Health Monitor</h2>
            <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest mt-1">Real-time External API Diagnostics</p>
          </div>
        </div>
          ...
          return (
            <motion.div 
              ...
              className={`p-6 rounded-3xl border transition-all ${
                ...
                isWait(health) ? 'bg-orange-500/10 border-orange-500/20' :
                isChecking ? 'bg-white/5 border-white/10' :
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-bold text-sm tracking-widest uppercase flex items-center gap-2">
                  {ep.name}
                  {ep.critical && <span className="text-[8px] bg-red-500 text-white px-2 py-0.5 rounded-full">CRITICAL</span>}
                </h3>
                {isChecking ? (
              ...
              <div className="flex items-end justify-between">
                <div>
                  <p className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 mb-1">Latency</p>
                  <p className={`text-2xl font-black italic ${isOnline ? 'text-green-500' : 'text-red-500'}`}>
                    {isChecking ? '...' : isOnline ? `${health.latency}ms` : 'ERR'}
                  </p>
                </div>
                <div className="text-right">
                   <p className="text-[8px] font-black uppercase tracking-widest text-gray-600">Last Ping</p>
  ...
  return (
    <motion.div 
      className={`p-6 rounded-3xl border transition-all ${
        ...
        isChecking ? 'bg-white/5 border-white/10' :
        'bg-orange-500/5 border-orange-500/20' // Firebase uses orange themed for distinction
      }`}
    >
      <div className="flex items-center justify-between mb-4">
        <h3 className="font-bold text-sm tracking-widest uppercase flex items-center gap-2">
          Firestore Database
          <span className="text-[8px] bg-red-500 text-white px-2 py-0.5 rounded-full">CRITICAL</span>
        </h3>
        {isChecking ? (
          <Activity className="w-5 h-5 text-gray-500 animate-spin" />
        ) : isOnline ? (
      ...
      <div className="flex items-end justify-between">
        <div>
          <p className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 mb-1">Latency</p>
          <p className={`text-2xl font-black italic ${isOnline ? 'text-orange-500' : 'text-red-500'}`}>
