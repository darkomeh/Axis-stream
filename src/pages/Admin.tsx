...
import { 
 ...
 getReports,
 resolveReport as resolveFirebaseReport,
...
export default function Admin() {
 ...
 setIsUpdating(true);
 try {
 await resolveFirebaseReport(id);
 showToast("Intel report closed.", "info");
 await fetchData();
 } catch (e) {
 ...
 if (!window.confirm("Permanently erase this intelligence record?")) return;
 setIsUpdating(true);
 try {
 await deleteFirebaseReport(id);
 showToast("Report purged from core.", "success");
 await fetchData();
 } catch (e) {
 ...
 <h2 className="text-fluid-2xl font-bold text-red-500">{pageError}</h2>
 ...
 <p className="text-fluid-sm font-semibold text-gray-500 tracking-wide mt-1">Cumulative platform time</p>
 </div>
 ...
 <p className="text-fluid-sm font-semibold text-gray-500 mb-2">Total Session Duration</p>
 <div className="flex items-baseline gap-3">
 <p className="text-fluid-4xl font-semibold text-white tracking-tight">
 ...
 <thead>
 <tr className="text-gray-500 text-fluid-sm font-semibold tracking-wide bg-white/2">
 ...
 <p className="text-fluid-sm text-gray-500 font-semibold tracking-wide">{user.email}</p>
 </div>
 </div>
 </td>
 <td className="px-8 py-6">
 ...
 <div>
 <label className="text-fluid-sm font-semibold tracking-wide text-gray-500 block mb-3 px-1">Brand Name</label>
 <input 
 type="text" 
 ...
 <div>
 <label className="text-fluid-sm font-semibold tracking-wide text-gray-500 block mb-3 px-1">Tagline / Intel</label>
 <input 
 type="text" 
 ...
 <div>
 <label className="text-fluid-sm font-semibold tracking-wide text-gray-500 block mb-3 px-1">API Provider / Core Connection</label>
 <select 
 ...
 <div className="flex items-center gap-4">
 <img src={user.avatar || undefined} className="w-14 h-14 rounded-2xl bg-transparent border border-white/10 shadow-xl" alt="" loading="lazy" />
 <div>
 <h4 className="font-bold text-fluid-lg leading-tight tracking-tight">{user.username}</h4>
 <p className="text-fluid-sm text-gray-600 font-semibold tracking-wide mt-1 ">{user.email}</p>
 </div>
 </div>
 </div>
 ...
 <p className="text-fluid-sm font-semibold ">{user.stats?.currentStreak || 0}</p>
 <p className="text-fluid-xs font-semibold text-gray-600 tracking-wide">Streak</p>
 </div>
 <div className="text-center">
 <p className="text-fluid-sm font-semibold ">{user.stats?.badges?.length || 0}</p>
 <p className="text-fluid-xs font-semibold text-gray-600 tracking-wide">Badges</p>
 </div>
 </div>

 <div className="flex items-center justify-between pt-4 border-t border-white/5 mt-4">
 <div className="flex flex-col">
 <div className="flex items-center gap-1 text-fluid-xs text-gray-500 font-semibold tracking-wide">
 ...
 <div>
 <h2 className="text-fluid-3xl font-semibold tracking-tight">Content Hub</h2>
 <p className="text-fluid-xs font-bold text-gray-500 tracking-wide">Pin priority media to global discovery</p>
 </div>
 </div>
 </div>
 ...
 <div>
 <h2 className="text-fluid-3xl font-semibold tracking-tight">Data Core</h2>
 <p className="text-fluid-xs font-bold text-gray-500 tracking-wide">Database records and deep analytics</p>
 </div>
 </div>
 </div>
 ...
 <tbody className="divide-y divide-white/5 text-fluid-sm font-medium">
 ...
 <td className="px-8 py-6 text-right"><span className="bg-white/10 px-2 py-1 rounded text-fluid-xs">Auth Required</span></td>
 </tr>
 <tr className="hover:bg-white/2 transition-colors">
 ...
 className="w-full text-fluid-sm font-semibold text-gray-500 hover:text-white tracking-wide transition-colors"
 >
 Clear Message
 </button>
 )}
 ...
 <p className="text-fluid-xs font-bold text-gray-500 tracking-wide">Global platform exception monitoring</p>
 </div>
 </div>
 </div>
 ...
 disabled={isUpdating}
 className="px-6 py-3 bg-green-500 text-white rounded-xl text-fluid-sm font-semibold tracking-wide hover:bg-green-600 transition-all flex items-center gap-2 whitespace-nowrap"
 >
 ...
 <div>
 <h4 className="text-fluid-lg font-semibold tracking-tight ">{ticket.subject}</h4>
 <p className="text-fluid-sm font-semibold text-gray-600 tracking-wide">From: {ticket.userName} ({ticket.userEmail})</p>
 </div>
 </div>
 <div className="flex items-center gap-3">
 <span className={`px-4 py-1.5 rounded-full text-fluid-xs font-semibold tracking-wide ${ticket.status === 'open' ? 'bg-orange-600/20 text-orange-400' : 'bg-green-600/20 text-green-400'}`}>
 {ticket.status}
 </span>
 <span className="text-fluid-sm font-bold text-gray-700">{new Date(ticket.createdAt?.toDate ? ticket.createdAt.toDate() : ticket.createdAt).toLocaleString()}</span>
 </div>
 </div>
 ...
 <div className="p-4 bg-green-500/5 border border-green-500/10 rounded-2xl text-fluid-sm font-semibold text-green-500 tracking-wide text-center">
 ...
 <p className="text-fluid-sm font-semibold tracking-wide">Support buffer empty</p>
 </div>
 )}
 </div>
 </div>
 ...
 <p className="text-fluid-lg font-bold text-white leading-relaxed">
 "{report.detail}"
 </p>

 <div className="flex flex-wrap gap-4 pt-4 border-t border-white/5">
 <div className="flex items-center gap-2 text-fluid-sm font-semibold text-gray-500 tracking-wide">
 <Users className="w-3.5 h-3.5" /> ID: {report.userId}
 </div>
 <div className="flex items-center gap-2 text-fluid-sm font-semibold text-gray-500 tracking-wide">
 <Clock className="w-3.5 h-3.5" /> {report.createdAt?.toDate ? report.createdAt.toDate().toLocaleString() : new Date(report.createdAt).toLocaleString()}
 </div>
 </div>
 </div>
 ...
 <span className="text-fluid-sm font-semibold tracking-wide text-gray-500">{log.type}</span>
 <span className="text-fluid-sm font-bold text-gray-700">{new Date(log.timestamp).toLocaleString()}</span>
 </div>
 <p className="text-fluid-sm font-medium text-gray-300 break-words">{log.detail}</p>
 </div>
 </div>
 ))}
 ...
 <p className="text-fluid-sm font-semibold tracking-wide">No transmissions recorded</p>
 </div>
 )}
 </div>
 </div>
...
function StatCard({ label, value, icon, color, isPercent, unit, durationSeconds }: { label: string, value: number, icon: React.ReactNode, color: string, isPercent?: boolean, unit?: string, durationSeconds?: number }) {
 ...
 <div>
 <p className="text-fluid-sm font-semibold text-gray-500 tracking-wide mb-1">{label}</p>
 <p className="text-fluid-4xl font-semibold tracking-tight">
