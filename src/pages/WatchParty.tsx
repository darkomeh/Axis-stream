...
export default function WatchPartyPage() {
      ...
      <div className="min-h-screen bg-black text-white flex flex-col relative overflow-hidden items-center justify-center p-6">
        ...
        >
          ...
          <p className="text-gray-400 text-sm mb-8 leading-relaxed">
            You've been invited to a live watch party! To join, chat with
            friends, and watch together, please sign in or create an account.
          </p>
  ...
  return (
    <div className="h-[100dvh] w-full bg-black flex flex-col md:flex-row overflow-hidden relative">
      ...
      {/* Main Player Area */}
      <div
        className={`relative flex flex-col bg-black z-40 transition-all duration-300 ${
          isChatOpen
      ...
      >
        ...
        <div className="absolute top-0 left-0 right-0 p-3 md:p-6 z-50 flex items-center justify-between bg-gradient-to-b from-black/85 via-black/45 to-transparent pointer-events-none">
          <div className="flex items-center gap-3 md:gap-4 pointer-events-auto">
            <button
              onClick={() => {
                if (party?.mediaTitle) {
                  navigate(`/details/${slugify(party.mediaTitle)}`);
                } else {
                  navigate("/");
                }
              }}
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="px-2 py-0.5 md:px-2.5 md:py-1 bg-brand text-white text-[9px] md:text-[10px] uppercase tracking-wider font-bold rounded-md">
                  Live
                </span>
                <h1 className="text-white font-semibold text-sm md:text-xl truncate max-w-[150px] md:max-w-md">
                  {party.mediaTitle}
                </h1>
              </div>
              <p
                className={`text-white/60 text-[10px] md:text-sm font-medium mt-0.5 flex items-center gap-1 ${isChatOpen ? "hidden md:block" : ""}`}
              >
                <span>Host:</span>
                <span className="font-semibold text-white">
                  {participants.find((p) => p.uid === party.hostId)?.displayName || "Unknown"}
                </span>
                {(participants.find((p) => p.uid === party.hostId)?.email === 'greatmayuku2@gmail.com' || (party.hostId === user?.id && user?.email === 'greatmayuku2@gmail.com') || participants.find((p) => p.uid === party.hostId)?.displayName?.includes('Busy')) && (
                  <MetaVerifiedBadge className="w-3.5 h-3.5 ml-0.5" />
                )}
              </p>
            </div>
          </div>
        ...
        {/* Video Player */}
        <div
          className={`w-full relative bg-black ${isChatOpen ? "h-full" : "aspect-video max-h-full"}`}
        >
          <VideoPlayer
            mediaData={mediaData}
        ...
        {isChatOpen && (
          ...
          >
            ...
            <div className="p-4 border-b border-white/5 flex gap-3 overflow-x-auto pb-4 custom-scrollbar">
              {participants.map((p) => {
                const isVerified = p.email === 'greatmayuku2@gmail.com' || p.displayName?.includes('Busy') || (p.uid === user?.id && user?.email === 'greatmayuku2@gmail.com');
                const displayNameToRender = isVerified ? '×͜× 𝙿𝚛𝚘𝚋𝚊𝚋𝚕𝚢 𝙱𝚞𝚜𝚢 永' : p.displayName;
                return (
                  <div
                    key={p.uid}
                  ...
                  >
                    ...
                    <span className="text-[10px] text-gray-400 max-w-[64px] truncate flex items-center gap-0.5 font-semibold">
                      {isVerified ? '×͜× 𝙿𝚛𝚘𝚋𝚊𝚋𝚕𝚢 𝙱𝚞𝚜𝚢 永' : displayNameToRender.split(" ")[0]}
                      {isVerified && (
                        <MetaVerifiedBadge className="w-2.5 h-2.5 flex-shrink-0" />
                      )}
                    </span>
                  </div>
                );
                  ...
                  >
                    {showAvatar ? (
                      <img
                        src={
                          msg.photoURL ||
                          `https://api.dicebear.com/7.x/avataaars/svg?seed=${msg.uid}`
                        }
                        alt=""
                    ...
                    >
                      {showAvatar && (
                        <span className="text-[10px] text-gray-500 mb-1 flex items-center gap-1 font-semibold">
                          {isMe ? "You" : displayNameToRender}
                          {isVerified && <MetaVerifiedBadge className="w-3 h-3" />}
                        </span>
                      )}
                      <div
                        className={`px-4 py-2.5 rounded-2xl text-[13px] leading-relaxed ${isMe ? "bg-brand text-white rounded-tr-sm" : "bg-white/10 text-gray-200 rounded-tl-sm"}`}
                      >
                        {msg.text}
                      </div>
                    </div>
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar bg-[#070707]">
              {messages.map((msg, i) => {
                ...
                return (
                  <div
                    key={msg.id}
                    className={`flex flex-col ${isMe ? "items-end" : "items-start"} gap-1 max-w-[85%] ${isMe ? "ml-auto" : ""}`}
                  >
                    <span className="text-[10px] text-white/40 flex items-center gap-1 font-semibold">
                      {isMe ? "You" : displayNameToRender}
                      {isVerified && <MetaVerifiedBadge className="w-2.5 h-2.5" />}
                    </span>
                    <div
                      className={`px-4 py-2.5 rounded-2xl text-[13px] shadow-sm leading-relaxed ${isMe ? "bg-brand text-white rounded-tr-none" : "bg-white/10 text-white rounded-tl-none"}`}
                    >
                      {msg.text}
                    </div>
                  </div>
            ...
            <div className="p-3 border-t border-white/10 bg-black/40">
              <form onSubmit={handleSendMessage} className="flex gap-2">
                ...
                <button
                  type="submit"
                  className="p-2.5 bg-brand text-white rounded-full"
                >
                  <Send className="w-4 h-4" />
                </button>
              </form>
          ...
          >
            ...
            >
              ...
              <p className="text-sm text-gray-400 mb-6">
                Send this link to your friends. They need to be logged in to
                join the party.
              </p>

              <div className="bg-black border border-white/10 rounded-xl p-3 flex items-center gap-3 mb-6">
                <div className="text-gray-300 text-sm truncate flex-1 font-mono">
              ...
              <button
                onClick={handleCopyInvite}
                className="w-full bg-brand hover:bg-brand-hover text-white font-bold py-3.5 rounded-xl transition-all"
              >
                Copy Party Link
              </button>
            </motion.div>
